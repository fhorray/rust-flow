...
import { Loader2, Terminal, CreditCard, LogOut, Check, ExternalLink, RefreshCw, Trash2, AlertTriangle } from "lucide-react";
...
export default function Dashboard() {
  ...
  const fetchProgress = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.progy.dev"}/progress/list`, {
        headers: {
  ...
  const handleResetCourse = async (courseId: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.progy.dev"}/progress/reset`, {
        method: "POST",
        headers: {
  ...
  const planLabel = isDual ? "LIFETIME + PRO" : hasLifetime ? "LIFETIME ACCESS" : isPro ? "PRO MEMBERSHIP" : "COMMUNITY";

  const planColor = (isPro || hasLifetime)
      ...
      <main className="pt-24 pb-20 px-4 md:px-6 max-w-6xl mx-auto space-y-12">
        ...
        <div className="grid gap-6 md:grid-cols-2">
            ...
            <CardContent className="px-6 pb-6 pt-0 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="mt-2 text-[10px] text-zinc-500 font-mono">ID: {session.user.id.substring(0, 12)}...</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-black/60 border-primary/20 relative overflow-hidden rounded-2xl p-2 shadow-[0_0_50px_-15px_rgba(251,146,60,0.1)] transition-transform hover:-translate-y-1" style={{ transitionDelay: '0.1s' }}>
            {(isPro || hasLifetime) && <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 blur-3xl rounded-full -mr-16 -mt-16"></div>}

            <CardHeader className="p-6 flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/50">License Status</CardTitle>
              ...
              <div className="mt-2 space-y-4">
                {plan === "free" && !hasLifetime && (
                  <p className="text-[11px] text-muted-foreground font-bold italic leading-relaxed">
                    ...
                    Access all free courses locally. Upgrade to unlock AI Mentor and Cloud Sync.
                  </p>
                )}
                ...
                {hasLifetime && (
                  <div className="space-y-3">
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-primary/5 border border-primary/10">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-white uppercase tracking-wider mb-1">Lifetime Access</p>
                        <p className="text-[10px] text-muted-foreground leading-snug">
                          Perpetual access to all current and future courses. No monthly fees for content.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {isPro && (
                  <div className="space-y-3">
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-purple-500/5 border border-purple-500/10">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-white uppercase tracking-wider mb-1">Master AI Enabled</p>
                        <p className="text-[10px] text-muted-foreground leading-snug">
                          Unlimited AI Mentor access (explanations, hints, generation). Recurring subscription.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <h2 className="text-xl font-black uppercase italic tracking-tighter">My Learning Path</h2>
          ...
          {loadingProgress ? (
            ...
            <div className="text-center py-12 bg-zinc-900/30 rounded-2xl border border-white/5 border-dashed">
              ...
              <h3 className="text-sm font-bold text-zinc-400 mb-1">No Active Courses</h3>
              <p className="text-xs text-zinc-600 mb-6 max-w-xs mx-auto">Start a course using the CLI (`progy init`) to see it appear here.</p>
              <CopyButton text="npx progy init" />
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {progressList.map((course) => (
                <Card key={course.courseId} className="bg-zinc-900/40 border-white/5 hover:border-primary/20 transition-all hover:-translate-y-1 group">
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                      <CardTitle className="font-bold text-sm tracking-tight text-zinc-200">{course.courseId}</CardTitle>
                      <Badge variant="secondary" className="text-[9px] font-mono bg-zinc-800 text-zinc-500">
                  ...
                  <CardFooter className="pt-0 flex justify-end">
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] text-zinc-600 hover:text-red-400 hover:bg-red-500/10 uppercase font-bold tracking-wider">
                          <Trash2 className="w-3 h-3 mr-1.5" /> Reset
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="border-zinc-800 bg-black/90 backdrop-blur-xl">
                        <DialogHeader>
                          <DialogTitle className="text-white">Reset Course Progress?</DialogTitle>
                          <DialogDescription className="text-zinc-400">
                            This will permanently delete all your progress for <strong>{course.courseId}</strong>. This action cannot be undone.
                          </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                          <Button variant="outline" onClick={() => { }} className="border-zinc-700 text-zinc-300">Cancel</Button>
                          <Button
                            variant="destructive"
