...
export function CommandPalette() {
  ...
  const { files, modules } = useMemo(() => {
    ...
    const allModules = fileTree.filter(n => n.type === 'dir' && /^\d{2}_/.test(n.name));
        ...
        >
          ...
          <Command.List className="flex-1 overflow-y-auto p-2 scroll-py-2">
            ...
            <Command.Group heading="Global Commands" className="mb-2">
              <div className="px-2 pb-1 text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Global Commands</div>
              <Command.Item
              ...
              >
                ...
                <kbd className="ml-auto text-[10px] bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-500 border border-zinc-700">Cmd+S</kbd>
              </Command.Item>
              <Command.Item
              ...
              <Command.Item
                onSelect={() => runCommand(() => openModuleSettings('.', 'Course Settings'))}
                className="flex items-center gap-3 px-3 py-2 text-sm text-zinc-300 rounded-lg aria-selected:bg-zinc-800 aria-selected:text-white cursor-pointer transition-colors"
              >
               ...
               <Command.Item
                onSelect={() => runCommand(() => openPage($router, 'home'))}
                className="flex items-center gap-3 px-3 py-2 text-sm text-zinc-300 rounded-lg aria-selected:bg-zinc-800 aria-selected:text-white cursor-pointer transition-colors"
              >
            ...
            {modules.length > 0 && (
              <>
                ...
                <Command.Group heading="Modules" className="mb-2">
                   <div className="px-2 pb-1 text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Modules</div>
                   {modules.map(module => (
                     <Command.Item
                      key={module.path}
                      value={`module-${module.name}`}
                      onSelect={() => runCommand(() => openModuleSettings(module.path, module.name))}
                      className="flex items-center gap-3 px-3 py-2 text-sm text-zinc-300 rounded-lg aria-selected:bg-zinc-800 aria-selected:text-white cursor-pointer transition-colors"
                    >
            ...
            {files.length > 0 && (
              <>
                ...
                <Command.Group heading="Files" className="mb-2">
                  <div className="px-2 pb-1 text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Files</div>
                  {files.map(file => (
                    <Command.Item
                      key={file.path}
                      value={file.path}
                    ...
                    >
                      ...
                      <span>{file.name}</span>
                      <span className="ml-auto text-[10px] text-zinc-600 truncate max-w-[150px]">{file.path}</span>
                    </Command.Item>
                  ))}
                </Command.Group>
              </>
          ...
          <div className="border-t border-zinc-800 px-3 py-2 flex items-center justify-between text-[10px] text-zinc-600 bg-zinc-900/50 shrink-0">
             ...
             <div>
               <span className="bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-700 mr-1">↑↓</span> to navigate
               <span className="bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-700 mx-1">↵</span> to select
               <span className="bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-700 ml-1">esc</span> to close
             </div>
          </div>
        </Command>
      </div>
