  ...
  return (
    <div className="flex items-center bg-zinc-800/60 rounded-md p-0.5">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1 rounded text-zinc-400 hover:text-zinc-200 text-[11px] font-medium transition-colors"
    ...
    <div className="h-screen w-screen flex flex-col bg-zinc-950 text-zinc-100 font-sans">
      ...
      <header className="h-11 border-b border-zinc-800/80 flex items-center px-4 bg-zinc-950 shrink-0">
        <button
          ...
          title={sidebarCollapsed ? 'Show sidebar (Ctrl+Shift+E)' : 'Hide sidebar (Ctrl+Shift+E)'}
        >
      ...
      <footer className="h-6 border-t border-zinc-800/80 flex items-center px-4 bg-zinc-900/50 shrink-0 text-[11px] text-zinc-500 gap-4">
        {activeTab ? (
          <>
            <span className="truncate max-w-[40%] font-mono">{activeTab.path}</span>
      ...
      {validationResult && (
        <div className={`fixed bottom-10 right-6 z-50 max-w-md rounded-xl border shadow-2xl p-4 ${validationResult.success
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-semibold ${validationResult.success ? 'text-emerald-300' : 'text-red-300'
                }`}>
                {validationResult.success ? 'Validation Passed!' : 'Validation Failed'}
              </p>
              {validationResult.success && validationResult.message && (
                <p className="text-xs text-emerald-400/70 mt-1">{validationResult.message}</p>
              )}
              {validationResult.errors && validationResult.errors.length > 0 && (
                <ul className="mt-2 space-y-1">
                  {validationResult.errors.map((err, i) => (
                    <li key={i} className="text-xs text-red-300/80 leading-relaxed">
                      {err}
                    </li>
                  ))}
                </ul>
