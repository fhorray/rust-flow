...
The core of this system is the `runner-server`. This is a lightweight HTTP server written in **Go** or **Rust** that runs _inside_ the Docker container. It is responsible for receiving the code payload, writing it to disk, executing the test command, and capturing the output.
...
const runHandler: ServerType<'/exercises/run'> = async (req) => {
  try {
    ...
    const useRemote =
      process.env.PROGY_REMOTE === 'true' ||
      currentConfig?.runner.type === 'remote';
...
async function handleRemoteExecution(exerciseId: string, config: any) {
  ...
  if (!token) {
    return Response.json({
      success: false,
      output:
        'Authentication required for remote execution. Run `progy auth login`.',
    });
  }
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      return Response.json({
        success: false,
        output: 'Network error connecting to runner service.',
      });
    }
  }
  ...
  return Response.json({
    success: false,
    output: 'Runner timed out (Cold Start). Please try again.',
  });
}
```
